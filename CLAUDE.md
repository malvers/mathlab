...
22. **Push auf `main` NUR mit Docs Freigabe — löst die „immer direkt compushen"-Default ab (Doc 2026-07-05).** Standard: **kein** automatischer Push auf `main`, auch nicht nach einem fertigen/getesteten Fix. Erst arbeiten/testen, dann auf Docs Freigabe warten. **Freigabe ist per Default EINMALIG (Doc 2026-07-06):** „push" bzw. „compush" heißt **genau diesen einen** Stand committen+pushen — danach wieder gated, vor dem **nächsten** Push erneut fragen. **NICHT** die Folge-Änderungen ungefragt hinterherpushen (v.a. nicht während wir noch an der Optik iterieren). **Dauer-Freigabe nur, wenn Doc sie ausdrücklich als solche ausspricht** — z. B. „pushen bis ich widerrufe" / „immer pushen bis Widerruf" → ab dann frei commit+push auf `main`, **bis Doc ausdrücklich zurücknimmt** („kein push" / „stopp push"). Im Zweifel: einmalig behandeln und fragen. Secret-Scan (Regel 18) + kein Force-Push bleiben immer. **Stop-Hook-Folge:** der Hook blockt das Turn-Ende nur bei *unpushed Commits* — im gated Zustand also **gar nicht committen**, sondern Änderungen im Working-Tree lassen und „bereit zum Push" melden. **OTWA/Cloud unberührt:** arbeitet auf eigenem `claude-*`-Branch und MUSS dorthin pushen (flüchtige Container, Regel 20); `main` war für OTWA ohnehin schon gated.
