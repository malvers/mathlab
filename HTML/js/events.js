...
// --- SPLITTER LOGIC (percentage-based on mobile) ---
...
if (splitter) {
    ...
    document.addEventListener('pointermove', (e) => {
        ...
        if (isMobile) {
            // Mouse position as viewport percentage (mobile stacked layout)
            let newPercentage = (e.clientY / window.innerHeight) * 100;
            // Never let the splitter shrink the canvas pane below 50% height
            newPercentage = Math.max(50, Math.min(newPercentage, 90));

            infoPanel.style.flex = "none";
            infoPanel.style.height = `${100 - newPercentage}vh`;
