...
${pts}
        ...
        function openPhotoLightbox(wp) { PhotoLayer.openLightbox(wp, waypoints); }
        ...
        function updateReburnButton() {
            ...
            if (!btn) return;
            const none = waypoints.filter(wpNeedsId).length === 0;
            btn.disabled = none;
        ...
        $('cam-fab').addEventListener('click', async () => {
            ...
            if (tracking) await addPhotoAt(img, ll, res.ai);   // pin on the active track
        ...
        async function addVoicePoint(rec, ll) {
            ...
            waypoints = waypoints.filter(x => x !== wp); // isolate from any later tracking session
        }
        ...
        (function () {
            ...
            async function finish() {
                ...
                if (tracking) await addVoiceAt(rec, ll);
