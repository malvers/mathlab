...
function createDraw20Stift({
    ...
    function redrawStift() {
        ...
        for (const s of strokes) {
            ...
            stiftCtx.strokeStyle = '#adff2f';
            stiftCtx.fillStyle = '#adff2f';
    ...
    function onStiftUp() {
        ...
        pushHistory();
        // Auto-extract: re-derive outlines after every completed stroke
        // so the user sees the updated outline immediately, without having
        // to click OUTLINES GENERIEREN.
        if (strokes.length > 0) extractStiftOutlines();
    }
    ...
    function renderStiftOutlines() {
        ...
        // Stift sits on the LEFT side of the tandem display → green
        // (matches #object-count-box border/colour). All outers same hue.
        const color = '#adff2f';
        cls.outers.forEach((o, i) => {
            const outerPoly = stiftContours[o.idx];
            if (!outerPoly || outerPoly.length < 2) return;

            // ── FILL: outer + holes as an evenodd subpath. Opaque (the
            // yellow strokes underneath get covered — POLYGONE FÜLLEN off
            // brings them back).
            ...
            // ── LINE: outer polygon (solid) — dark red for now.
            ...
            // ── POINTS: vertex dots — blue for now.
