/* pagode-auth.js — optional security gate in front of START (single source, like pagode-ble.js).
   Three selectable modes, remembered in localStorage under 'pagode-auth-mode':
...
(function (global) {
    ...
    var MODES = [
        { id: 'off', label: 'Aus', hint: 'kein Schutz' },
        { id: 'pin', label: 'PIN', hint: 'eigene PIN, überall' },
    ...
    function bioPlugin() {
        return (global.Capacitor && global.Capacitor.Plugins && global.Capacitor.Plugins.BiometricAuth) || null;
    }
    ...
    async function bioAuth() {
        ...
        try {
            ...
            await p.authenticate({
                reason: 'Pagode starten',
                androidTitle: 'Pagode entsperren',
                androidSubtitle: 'Motor starten',
        ...
        card.innerHTML = '<div class="pgauth-title">Sicherheit</div>' +
            MODES.map(function (m) {
                return '<div class="pgauth-opt' + (m.id === 'off' ? ' off' : '') + (getMode() === m.id ? ' sel' : '') + '" data-id="' + m.id + '">' +
                    '<span class="l">' + m.label + '</span><span class="h">' + m.hint + '</span></div>';
            }).join('') +
