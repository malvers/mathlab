...
## FEAT-35 — Schild selbst erfassen (Tap-Panel) + lokaler Override-Store (+ späterer OSM-Export) 🏗️ Phase 1 gebaut · Prio 2 (Doc, „sounds like a plan" 2026-07-04)
**Phase 1 gebaut (2026-07-04):** `HTML/js/tracker-signpanel.js` — **Doppel-Tap auf die Karte** öffnet links eine vertikale Säule runder Schild-Chips (5/30/50/70/100, „30 werktags 6–20", Überholverbot, Maut, „× hier löschen"). Tap → **lokales Override** in `tracker-speedlimit.js` (`setOverride`/`clearOverridesNear`, `localStorage`, 70 m Radius am Tap-Punkt), das über Profil + Live-OSM **gewinnt** und sofort am `#speed-sign` erscheint; zeitbedingtes nutzt die OSM-`{base,rules}`-Struktur → Fenster-Label greift 1:1, „werktags"=Mo–Sa. Advisory (Überholverbot/Maut) bleibt über OSM sticky (`overrideAt` in `query()` ge-OR-t). Reload-fest. **→ offen:** Phase 2 (Supabase-DB), Phase 3 (Upload OSM/Overpass, OAuth), Feld-Check am Gerät.
