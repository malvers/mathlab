...
// ===== Sky interaction (hover, zoom, tooltips) =====
        ...
        function setStarHover(clientX, clientY) {
            ...
            // Over the plate rectangle → suppress red-line + white-star hover so the plate stays readable;
            // outside it, normal star/constellation hovering works.
            starHover.on = !_ptInArtQuad(starHover.x, starHover.y);
        }
        canvas.addEventListener('mousemove', (e) => setStarHover(e.clientX, e.clientY));
        // Track which constellation name label the pointer is currently over (cyan highlight) — off when over the plate.
        canvas.addEventListener('mousemove', (e) => {
            ...
            if (_ptInArtQuad(_x, _y)) { hoveredLabelId = null; return; }
            let _found = null;
        ...
        function hoverPick(clientX, clientY) {    // priority: zodiac → deep-sky → star/constellation
            // Over the plate rectangle → no tooltips (the plate is the focus). Outside it, normal hover-picking.
...
// ===== segDist + drawStarfield =====
        ...
        function drawStarfield(w, h, cx, cy) {
            ...
            // CONSTELLATION-ART (Jamieson plates 1822) — shown as a simple rectangular "card" at the selected
            // constellation's centroid. Earlier we tried to warp the plate exactly onto the dome via TPS through
            // calibrated anchors; the result never quite matched (the plates are historic conic projections drawn
            // by hand, individual stars are off by 1–3 px on the plate itself). The rectangular display is honest:
            // here's the historical art for THIS constellation, no fake precision. (Toggle 'm'.)
            if (showArt && selectedArtId && typeof CONSTELLATION_ART !== 'undefined') {
                for (const art of CONSTELLATION_ART) {
                    ...
                    if (!art._imgReady || !art._img.naturalWidth) continue;
                    // Place the plate centered on the constellation's centroid (the same point used for its label).
                    // If the centroid is below the horizon the plate is hidden; otherwise the rectangle is clamped to
                    // stay fully on-canvas so it never falls off-screen as the sky rotates.
                    const _cent = (typeof CON_CENTROIDS !== 'undefined') ? CON_CENTROIDS[art.id] : null;
                    if (!_cent) continue;
                    const _cp = proj(_cent[0], _cent[1]);
                    if (!_cp || !_cp[2]) { artHoverQuad = null; continue; }            // centroid below horizon → no plate
                    const W = art._img.naturalWidth, H = art._img.naturalHeight;
                    const _aspect = W / H;
                    // Target size: fit the rectangle into ≈55 % of the dome diameter, aspect-preserved. Big enough to
                    // read the engraving detail, small enough that the surrounding constellations stay visible.
                    const _targetH = (dome ? Rdome : Math.min(w, h) * 0.5) * 0.55;
                    const _targetW = _targetH * _aspect;
                    let _rx = _cp[0] - _targetW / 2, _ry = _cp[1] - _targetH / 2;
                    _rx = Math.max(8, Math.min(w - _targetW - 8, _rx));               // clamp inside canvas with a small margin
                    _ry = Math.max(8, Math.min(h - _targetH - 8, _ry));
                    // Hover-detect: simple rectangle hit test → ease alpha toward 1.0 over artAlpha while pointer is inside.
                    artHoverQuad = [
                        [_rx, _ry], [_rx + _targetW, _ry],
                        [_rx + _targetW, _ry + _targetH], [_rx, _ry + _targetH]
                    ];
                    let _hover = false;
                    if (typeof fkMouse !== 'undefined' && fkMouse) {
                        _hover = fkMouse.x >= _rx && fkMouse.x <= _rx + _targetW
                              && fkMouse.y >= _ry && fkMouse.y <= _ry + _targetH;
                    }
                    const _target = _hover ? 1 : artAlpha;
                    if (art._alphaEff == null) art._alphaEff = artAlpha;
                    art._alphaEff += (_target - art._alphaEff) * 0.15;
                    // Draw the plate as a plain rectangle — no warping, no mesh, no calibration. Surrounded by a
                    // soft cyan frame so it reads as an overlay card rather than part of the sky.
                    ctx.save();
                    ctx.globalAlpha = a * art._alphaEff;
                    ctx.drawImage(art._img, _rx, _ry, _targetW, _targetH);
                    ctx.strokeStyle = `rgba(0, 210, 255, ${0.45 * a * art._alphaEff})`;
                    ctx.lineWidth = 1.5;
                    ctx.strokeRect(_rx + 0.5, _ry + 0.5, _targetW - 1, _targetH - 1);
                    ctx.restore();
                }
            }
