...
<!-- Page markup, scoring, statistics and the teacher dashboard live in
     ...
     Generated by tools/build_infotest12.py - edit the questions there. -->
<script>
window.QUIZ = %(json)s;
</script>
...
def build(slug, nr, kw, topic, qs):
    qid = f"fos12-w{nr:02d}-{slug}"
    questions = []
    for i, (q, right, wrong, steps) in enumerate(qs, 1):
        assert len(wrong) == 3 and len({right, *wrong}) == 4, (qid, i)
        pos = int(hashlib.sha256(f"{qid}#{i}".encode()).hexdigest(), 16) % 4
        opts = list(wrong)
    ...
    quiz = {
        ...
        "subtitle": f"Fachoberschule · Klasse 12 · Woche {nr}, KW {kw} · 10 Fragen zu den Essentials "
                    f"der Woche · genau eine Antwort pro Frage",
        "dashSub": f"FOS · Klasse 12 · KW {kw} · Live-Auswertung: anonyme Einzelscores + Gruppenleistung pro Frage",
        "back": "svp/informatik/fos12.html",
        "questions": questions,
    }
    path = os.path.join(HTML, f"infotest12-{slug}.html")
