...
/* --- HUD ---------------------------------------------------------------- */
#hud{position:fixed;right:14px;bottom:12px;display:flex;gap:10px;align-items:center;
  font-family:Orbitron,sans-serif;font-size:12px;color:var(--ink);z-index:9}
#hud button{font:inherit;color:inherit;background:rgba(255,255,255,.72);border:0;
...
function paint(){
  ...
  document.getElementById('no').textContent = (si + 1) + ' / ' + slides.length;
...
<div id="bar"></div>
<div id="hud"><span id="no"></span><button id="full">Vollbild</button></div>
