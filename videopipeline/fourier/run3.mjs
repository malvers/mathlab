// Fourier demo — step 3: cut the take into scenes, mix the narration, let D-ID speak it
...
const scenes = ORDER.map((k) => {
  ...
  return { name: `fourier_${k}`, src, segments: [[t0, tEnd]], len, audio: `${OUT}/${k}.mp3`, pad: len - (tEnd - t0) };
});
...
const CHAPTER = {
  s1: 'Erst das Ende', s2: 'Zwei Kreise, ein Stift', s3: 'Der Gegenläufer',
  s4: 'Alles auf einmal', s5: 'Niemand rät die Kreise', s6: 'Zwei Kreise, die nichts tun',
  s7: 'Fast 99 Prozent — und ein Kreis', s8: 'Vier Kreise, die nichts ändern',
  s9: 'Die Ecke, die keine wird', s10: 'Das Herz und der Drehsinn',
  s11: 'Drei Formen, dieselben zwei Kreise', s12: 'Weglassen ist der Trick',
};
let cAt = 0;
const chapters = [];
for (const s of scenes) {
...
const outros = await recordOutros({
  outDir: OUT, qrUrl: 'https://docalvers.de/fourier.html', cards: ['logo'],
});
const total = scenes.reduce((a, s) => a + s.len, 0) + outros.reduce((a, f) => a + dur(f), 0);
compose(scenes.map((s) => s.name), bubbles,
  { outDir: OUT, out: `${OUT}/fourier-demo-1440p.mp4`, size: 380, outros,
