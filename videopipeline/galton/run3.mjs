// Galton demo — step 3: cut the take into scenes, mix the narration, let D-ID speak it
...
// five cards out of their own clips. The film order interleaves them.
//
...
const CARDS = new Set(['s4', 's5', 's6', 's8', 's9']);
const LAB_ORDER = ORDER.filter((k) => !CARDS.has(k));
// A lab scene normally runs to the next lab mark. Scene 7 is the exception: after it
// run2 fills the board for scene 10, which takes seventeen seconds that belong to no
// scene at all. 's7ende' marks where the picture stops being scene 7.
const END_MARK = { s7: 's7ende' };
...
const scenes = ORDER.map((k) => {
  ...
  if (CARDS.has(k)) {
    ...
    src = `${OUT}/main.mp4`;
    t0 = marks[k];
    const own = END_MARK[k] && marks[END_MARK[k]] !== undefined ? marks[END_MARK[k]] : null;
    const nx = LAB_ORDER[LAB_ORDER.indexOf(k) + 1];
    tEnd = Math.min(own ?? (nx ? marks[nx] : marks.end + 1.6), mainDur);
  }
  const len = Math.max(tEnd - t0, durs[k] + 1.2);        // 0.5 s lead + tail room
  return { name: `galton_${k}`, src, segments: [[t0, tEnd]], len, audio: `${OUT}/${k}.mp3`, pad: len - (tEnd - t0) };
});
...
const CHAPTER = {
  s1: 'Eine Kugel, achtzehn Münzwürfe', s2: 'Ein Dutzend Kugeln: nichts',
  s3: 'Tausend Kugeln: die Glocke', s4: 'Warum ausgerechnet diese Form',
  s5: 'Die Mitte ist kein Glück', s6: 'Was das Außenfach kostet',
  s7: 'Doppelt so viele Nägel', s8: 'Es liegt nicht am Brett',
  s9: 'Und wo es nicht gilt', s10: 'Achtzehn Münzwürfe',
};
let cAt = 0;
const chapters = [];
for (const s of scenes) {
...
// One D-ID clip per narration block, so Solita's mouth stays in sync across the whole
// film. At roughly 190 s of speech this is a single talk — one clip, no seam.
const MAX_TALK = 280;
const groups = [];
...
const outros = await recordOutros({ outDir: OUT, qrUrl: 'https://docalvers.de/galtonboard.html' });
const total = scenes.reduce((a, s) => a + s.len, 0) + outros.reduce((a, f) => a + dur(f), 0);
compose(scenes.map((s) => s.name), bubbles,
  { outDir: OUT, out: `${OUT}/galton-demo-1440p.mp4`, size: 380, outros,
