// Reaction-Diffusion demo — step 3: cut the single take into fifteen scenes, mix the
...
// s14 is missing on purpose: the scene it was written for could not be filmed
// honestly (see narration.mjs), so it was cut before the shoot, not after.
...
const scenes = ORDER.map((k, i) => {
  ...
  return { name: `rd_${k}`, src: `${OUT}/main.mp4`, segments: [[t0, tEnd]], len,
...
const CHAPTER = {
  s1: 'Ein Tropfen, sonst nichts', s2: 'Es kommt von selbst',
  s3: 'Zwei Stoffe, einer unsichtbar', s4: 'Vier Regeln, mehr nicht',
  s5: 'Die zwei Regler', s6: 'Zwei Zahlen, sechzehn Welten',
  s7: 'Und meistens passiert gar nichts', s8: 'Man kann es nicht kaputtmalen',
  s9: 'Der Rand ist keiner', s10: 'Warum überhaupt Muster?',
  s11: 'Die Probe: den Unterschied wegnehmen', s12: 'Und zurück',
  s13: '1952', s15: 'Zum Selberzüchten',
};
let cAt = 0;
const chapters = [];
for (const s of scenes) {
...
const outros = await recordOutros({ outDir: OUT, qrUrl: 'https://docalvers.de/reaction-diffusion.html' });
const total = scenes.reduce((a, s) => a + s.len, 0) + outros.reduce((a, f) => a + dur(f), 0);
compose(scenes.map((s) => s.name), bubbles,
  { outDir: OUT, out: `${OUT}/reaction-diffusion-demo-1440p.mp4`, size: 380, outros,
