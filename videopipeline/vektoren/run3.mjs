// Vektoren demo — step 3: cut the four takes into 24 scenes, lay Solita's voice under
...
const TAKES = {
  takeA: ['s1', 's2', 's3', 's4', 's5', 's6', 's7'],       // die Ebene
  takeB: ['s8', 's9', 's10', 's11', 's12', 's13', 's14'],  // der Raum
};
const ORDER = Object.values(TAKES).flat();

const CHAPTER = {
  s1: 'Punkt oder Pfeil?', s2: 'Spitze minus Fuß', s3: 'Spitze an Fuß',
  s4: 'Strecken und spiegeln', s5: 'Wie lang ist ein Pfeil?', s6: 'Jeden Punkt erreichen',
  s7: 'Wenn beide dasselbe sagen', s8: 'Ab in den Raum', s9: 'Der Winkelmesser',
  s10: 'Der Pfeil, der auf beiden steht', s11: 'Ebenen lesen', s12: 'Trifft sie oder nicht?',
  s13: 'Wie weit ist weg?', s14: 'Abspann',
};
...
const outros = await recordOutros({ outDir: OUT, qrUrl: 'https://docalvers.de/vektoren.html' });
const total = scenes.reduce((a, s) => a + s.len, 0) + outros.reduce((a, f) => a + dur(f), 0);
const out = `${OUT}/vektoren-${process.env.DID ? 'demo' : 'voice'}-1440p.mp4`;
