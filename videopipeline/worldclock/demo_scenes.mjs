// Auto-derived from demo.mjs — narration + scenes as an importable module.
  ...
  { name: 's7', url: BASE + '?lang=de', run: async (p, { mark }) => {   // time-lapse until a GOOD ISS pass, zoom-follow it
      ...
      for (let i = 0; i < 1400; i++) {   // up to ~6 min; skip passes below 20° max elevation
        ...
        const st = await p.evaluate(() => {
          ...
          return { cap, iss, alt: info ? info.alt : null, maxEl: info ? info.maxEl : null };
        }).catch(() => ({ cap: '', iss: false }));
        const capKey = (st.cap.match(/flyby — (\w+)/) || [])[1] || 'clean';
        if (capKey !== p.__lastCap) { p.__lastCap = capKey; mark('cap-' + capKey); }
...
export default { NARRATION, SCENES };
